  ...
  return logs
    ...
    .map((log) => {
      ...
      return {
        ...
        targetBundleName: log.target_bundle_name || log.rule_runs?.[0]?.target_bundle_name || null,
        focusStrategy: log.focus_strategy || log.rule_runs?.[0]?.focus_strategy || null,
        readbackStrategy: log.readback_strategy || log.rule_runs?.[0]?.readback_strategy || null,
...
export async function buildDashboardData() {
  ...
  return {
    ...
    morningWorkflow: [
      ...
      { label: '2. Read the release summary', description: 'Use the 0.2.0 RC summary as the latest handoff note.', path: 'notes/v2-upgrade-summary.md' },
